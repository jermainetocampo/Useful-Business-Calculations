...
def shift_letter(letter, shift):
    ...
    *Note: the single underscore `_` is used to acknowledge the presence
    ...
    if letter  == " ":
        ...
        if (shift + index) > 25:
            newshift = (shift + index)%26
            if  newshift > 1 and newshift + index < 25:
                new = newshift + index
            if shift + index >= 25:
                new = newshift
        else:
...
def caesar_cipher(message, shift):
    ...
    for x in range(length):
        ...
        if words[x]  == " ":
            ...
            index = alphabet.index(words[x])
            if (shift + index) > 25:
                newshift = (shift + index)%26
                if  newshift > 1 and newshift + index < 25:
                    new = newshift + index
                    words[x] = alphabet[new]                    
